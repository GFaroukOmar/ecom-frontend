...
const VariationsForm = ({ attributes = [], parentVariations = [], parentSetState}) => {
    ...
    }, [attributes,parentVariations]);

    useEffect(() => {
        if (JSON.stringify(localVariations) !== JSON.stringify(parentVariations)) {
    ...
    const addVariation = () => {
        ...
        const newVariation = {
            ...
            attributeValues: attributes.map(attribute => ({
                ...
                value: attribute.values[0].value, // Default value for the attribute
    ...
    const addAllPossibleVariations = () => {
        if (attributes.some(attribute => attribute.values.length === 0)) {
            alert("Please ensure all attributes have at least one value before generating variations.");
            return;
        }

        // Helper function to compute the cartesian product of attributes
        ...
        // Extract attribute values as an array of arrays
        ...
        const newVariations = attributeCombinations.map((combination) => ({
            variationId: Date.now() + Math.random(), // Unique ID using timestamp and random
            quantity: 100, // Default quantity
            variationImage: null, // Default image
            attributeValues: attributes.map((attribute, index) => ({
                attributeId: attribute.attributeId,
                valueId:combination[index].valueId,
                value: combination[index].value, // Assign value based on the combination
                attributeName: attribute.attributeName,
            })),
        }));

        // Merge new variations with existing ones
    ...
    const deleteVariation = (variationId) => {
        const updatedVariations = localVariations.filter((variation) => variation.variationId !== variationId);
        setLocalVariations(updatedVariations);
        // parentSetState(updatedVariations);
    };

    // Change the selected attribute value for a variation
    ...
    // Set quantity for a variation
    const setVariationQuantity = (variationId, quantity) => {
        const updatedVariations = localVariations.map((variation) =>
            variation.variationId === variationId ? { ...variation, quantity } : variation
        );
    ...
    return (
        <div>
            <div className="mb-3 d-flex gap-2">
                ...
                <button onClick={addAllPossibleVariations} className="btn btn-success">
                    Create All Possible Variations
                </button>
            </div>
                        ...
                        <div key={index} className="accordion-item">
                            <h2 className="accordion-header" id={`heading-${index}`}>
                                <button
                                    ...
                                    aria-expanded={index === 0 ? "true" : "false"} // First item open by default
                            ...
                            >
                                <div className="accordion-body">
                                    ...
                                    <div className="d-flex container gap-3 align-items-center">
                                        {attributes.map((attribute, attributeIndex) => (
                                            <div key={attributeIndex} className="mb-2">
                                                <label htmlFor={`attribute-${attribute.attributeId}`} className="form-label">
                                    ...
                                    {/* Quantity and Image Select */}
                                    <label className="form-label">Variation Information</label>
                                    <div className="container d-flex gap-3 align-items-start">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    setVariationQuantity(variation.variationId, parseInt(e.target.value))
                                                }
                                            />
                                        </div>
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <DragAndDropFiles
                                                onImagesChange={(image) =>
                                                    setLocalVariations(localVariations.map(localVariation =>
                                                        localVariation.variationId === variation.variationId
                                                            ? { ...localVariation, variationImageUrl: image.imageUrl }
                                                            : localVariation
                                                    ))}

                                                initialImages={[{ imageUrl: variation.variationImageUrl, isMainImage: true }]}
                                            />
                                        </div>
                                    </div>
                                </div>
...
const AttributeField = ({onRemoveAttribute, onChangeAttributeName, attributeName, attributeId, values = [],
    ...
    const showValueErrors = values.map((value) => !value.valueText); // Check for empty values
